    ...
    Craft.Smith.Menu = Garnish.Base.extend({
        ...
        checkPaste: function () {
            ...
            try {
                ...
                var fieldHandleCopiedBlock = data.field;
                var regex = new RegExp("leistungen(Morgen|Mittag|Abend)")

                // Find copy data for this field
                if (data && regex.test(fieldHandle) && regex.test(fieldHandleCopiedBlock)) {
                    canPaste = true;
                } else {
        ...
        pasteBlock: function (e, data) {
            try {
                ...
                if (regex.test(data.field)) {
                    var id = this.$matrixField.attr('id');
                    if (id.includes('Morgen')) {
                        data.field = 'leistungenMorgen';
                    } else if (id.includes('Mittag')) {
                        data.field = 'leistungenMittag';
                    } else if (id.includes('Abend')) {
                        data.field = 'leistungenAbend';
                    }
                }
